...
class HalmaGame(object):
    ...
    def canJump(self, (pawnX, pawnY), color):
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                if i == 0 and j == 0:
                    continue
                if (i + j > 0 and opponentsColor == 'B') or (i + j < 0 and opponentsColor == 'W'):
                    x = pawnX + i
                    y = pawnY + j
                    if (x, y) in self.board[opponentsColor]:
                        jumpX = x - i * 2
                        jumpY = y - j * 2
                        if jumpX < 0 or jumpX > 15 or jumpY < 0 or jumpY > 15:
                            continue
                        elif (jumpX, jumpY) in self.board['total']:
                            continue
                        return True
